import asyncio
from core.models import db_helper, User, Profile, Post, Base, OrderProductAssociation
...
async def get_user_by_username(session: AsyncSession, username: str) -> User | None:
    ...
    print("found_user", username, user)
    return user
...
async def main_relations(session: AsyncSession):
        user_lisa = await create_user(session=session, username="Lisa")
...
async def create_orders_and_products(session: AsyncSession):
    ...
    mouse = await create_product(session, name="Mouse", description="Gaming mouse", price=123)
    keyboard = await create_product(session, name="Keyboard", description="Gaming keyboard", price=179)
    display = await create_product(session, name="Display", description="Office display", price=299)
...
async def create_order(session: AsyncSession, promocode: None | str = None) -> Order:
...
async def demo_get_orders_with_products_through_assoc(session: AsyncSession):
...
async def demo_m2m(session: AsyncSession):
    ...
    # await demo_get_orders_with_products_through_assoc(session)
